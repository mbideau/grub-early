...
debug "Configurations files to parse: %s" "$(echo "$conf_files"|sed "s#$GRUB_MEMDISK_DIR\\?/##g")"
# shellcheck disable=SC2086
modules_cmd="$(for cmd in $(get_command_list $conf_files); do
get_grub_cmd_modules "$cmd"; done|tr ' ' '\n'|sort -u|tr '\n' ' '|trim)"
...
modules_theme="$(get_modules_from_theme_files \
    $(find "$GRUB_THEMES_DIR" -name "$GRUB_THEME_FILENAME" -o -name "$GRUB_THEME_INNER_FILENAME" ))"
debug "Modules required by themes: %s" "$modules_theme"
modules="$modules_crypto $modules_disk $modules_fs $modules_gfxmenu"
debug "Modules required by devices (current host): %s" "$modules"
# TODO include modules required by other hosts (specially their devices requirements)
modules="$(echo "$modules $modules_cmd $modules_theme"|tr ' ' '\n'|sort -u|tr '\n' ' '|trim)"
